...
interface PredictionData {
  ...
  lon: number;
  riesgo: string;
  predicciones: Record<string, number>;
  factores: {
    humedad: number;
    temperatura: number;
    saturacion_hospitalaria: number;
  };
}

const App: React.FC = () => {
  const [activeTab, setActiveTab] = useState("prediction");
  const [datosPrediccion, setDatosPrediccion] = useState<Record<string, PredictionData>>({});
  const [estadoSeleccionado, setEstadoSeleccionado] = useState<string | null>(null);
  const [dataEstado, setDataEstado] = useState<PredictionData | null>(null);

  useEffect(() => {
    fetch(`${import.meta.env.BASE_URL}datos_prediccion_candida_auris.json`)
      .then(res => res.json())
  ...
  useEffect(() => {
    ...
    Object.entries(datosPrediccion).forEach(([estado, datos]) => {
