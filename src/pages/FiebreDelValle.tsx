...
const stateData: Record<string, {
  ...
  growth: number;
  aridityLevel: string;
  aridityPercentage: number;
  dustStormLevel: string;
  dustStormPercentage: number;
}> = {
        ...
        const option = {
          ...
          tooltip: {
            ...
            formatter: (params: echarts.TooltipComponentFormatterCallbackParams) => {
              ...
              const data = stateData[point.name];
              if (data) {
                return `${point.name}<br/>Riesgo: ${data.risk}<br/>Casos: ${data.cases}<br/>Crecimiento: +${data.growth}%`;
              }
              return point.name;
            }
          },
